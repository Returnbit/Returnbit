...
*Setup instructions for a Gitian build of Returnbit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Returnbit
...
- [Building Returnbit](#building-returnbit)
...
Clone the git repositories for returnbit and Gitian and then checkout the returnbit version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/returnbit-project/returnbit.git
cd returnbit
...
Follow the instructions in [doc/release-process.md](release-process.md) in the returnbit repository
...
To build Returnbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the returnbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/returnbit/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/returnbit-project/returnbit
...
For example:
```bash
URL=https://github.com/user/returnbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit returnbit=${COMMIT} --url returnbit=${URL} ../returnbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit returnbit=${COMMIT} --url returnbit=${URL} ../returnbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit returnbit=${COMMIT} --url returnbit=${URL} ../returnbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/returnbit-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/returnbit-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/returnbit-build.assert
```
...
[returnbit/gitian.sigs](https://github.com/returnbit-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
