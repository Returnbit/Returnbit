...
#if defined(NDEBUG)
# error "Returnbit cannot be compiled without assertions."
#endif
...
CScript RTRUST_SCRIPT;



const string strMessageMagic = "Returnbit Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("returnbit-scriptch");
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("FindBlockPos: Pre-allocating up to position 0x%x in blk%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool FindUndoPos(CValidationState &state, int nFile, CDiskBlockPos &pos, unsigned int nAddSize)
{
    ...
    if (nNewChunks > nOldChunks) {
        if (CheckDiskSpace(nNewChunks * UNDOFILE_CHUNK_SIZE - pos.nPos)) {
            ...
            if (file) {
                LogPrintf("FindUndoPos: Pre-allocating up to position 0x%x in rev%05u.dat\n", nNewChunks * UNDOFILE_CHUNK_SIZE, pos.nFile);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Returnbit: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Returnbit: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool InitBlockIndex() {
    ...
    RTRUST_SCRIPT << OP_DUP << OP_HASH160 << ParseHex(RTRUST_ADDRESS) << OP_EQUALVERIFY << OP_CHECKSIG;
